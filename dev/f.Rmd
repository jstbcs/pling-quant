...
\mu_{ijk} &= \text{logit}^{-1}(\gamma_{ij} + (1 - 2 \gamma_{ij}) \pi_{ijk}),\\
\end{align*}
...
\nu &\sim \mbox{Normal}(0, 5^2),\\
\sigma_\alpha^2 &\sim \mbox{Inverse-Gamma}(2, .2).\\
\delta &\sim \mbox{Normal}(0, 5^2).\\
\sigma^2 &\sim \mbox{Inverse-Gamma}(2, .2).\\
\end{align*}
...
corrplot(M3, type = "upper", title = "Guessing", mar = c(0,0,2,0))
```
...
make.cor.plot(cbind("Vagueness" = palpha5, "Threshold" = pbeta5, "Error" = pgamma5), title = "Most")
```
...
legend("bottomright", legend = c("Ideal responding", "Vague", "Response error", "Vague + response error"), fill = c("darkgray", "slateblue", "firebrick", "darkgreen"), bty = "n")
```
...
```{r check-participants-exp2}
dat$above <- dat$percent > 50
...
```{r run-logmod-2, cache = T, eval = F, results = "hide"}
rerun <- T
logmodfit <- myRunner(predat
                      , iter = 2000
                      , warmup = 850
                      ...
                      , nchains = 6)
save(logmodfit, file = "outstudy2f.rda")
```
...
```{r inspect-logmod-2, fig.asp=1.2}
pEst <- as.matrix(logmodfit)
dim(pEst)
...
```{r calc-thresholds-2, fig.width=4, fig.asp=1}
make.cor.plot <- function(mat, ...){
  M <- cor(mat)
  corrplot(M, type = "upper", mar = c(0,0,2,0), ...)
}
...
```{r thresholds-exp2-b, fig.asp=1.5}
layout(matrix(c(1,2,3,3), ncol = 2, byrow = T), heights = c(.4, .6))
