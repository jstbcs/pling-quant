...
model {
  ...
  for (i in 1:I)
    alpha[i] ~ lognormal(gamma, sigmaalpha);
  for (n in 1:N)
...
myRunner <- function(standat, iter = 1000, warmup = 400, mod, nchains = 4, ...){
  ...
  fit <- sampling(mod, verbose=T,
                  ...
                  pars = c("delta", "gamma", "beta", "alpha", "sigma2", "sigma2alpha")
                  , ...)
  return(fit)}
...
pps1 <- curve.calc(c(pdelta1, exp(pgamma1)))
pps2 <- curve.calc(c(pdelta2, exp(pgamma2)))
pps3 <- curve.calc(c(pdelta3, exp(pgamma3)))
pps4 <- curve.calc(c(pdelta4, exp(pgamma4)))
pps5 <- curve.calc(c(pdelta5, exp(pgamma5)))
...
res1 <- apply(cbind(pbeta1, palpha1), 1, curve.calc)
matplot(res1, type = "l", lty = 1, col = qcols[1], main =  "Few")
res2 <- apply(cbind(pbeta2, palpha2), 1, curve.calc)
matplot(res2, type = "l", lty = 1, col = qcols[2], main =  "Fewer than half")
res3 <- apply(cbind(pbeta3, palpha3), 1, curve.calc)
matplot(res3, type = "l", lty = 1, col = qcols[3], main =  "Many")
res4 <- apply(cbind(pbeta4, palpha4), 1, curve.calc)
matplot(res4, type = "l", lty = 1, col = qcols[4], main = "More than half")
res5 <- apply(cbind(pbeta5, palpha5), 1, curve.calc)
