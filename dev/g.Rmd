...
## participant 27
dat27 <- subset(dat, workerid == 27)
dat27$percentabove <- factor(c(dat27$percent > 50), labels = c("below50", "above50"))
tapply(as.numeric(as.factor(dat27$response)) - 1, list(dat27$quant, dat27$percentabove), mean, na.rm = TRUE)
...
```{r CrIs-3, eval = T}
for(i in 1:ncol(mdiff)){
  mdiff[,i] <- apply(cbind(mbeta3[,i], mbeta5[,i]), 1, function(x) x[2] - x[1])
}
pCIs <- apply(mdiff, 2, quantile, probs = c(.025, .975)) 
pThresh <- apply(mdiff, 2, mean)
```
...
plotCI(1:I, pThresh[indord]
       , li = pCIs[1, indord], ui = pCIs[2, indord]
       , pch = 21, pt.bg = qcols[res.clust.thr_vag_pars$cluster]
       , ylab = "Most - Many"
       , xlab = "Participant")
abline(h = 0)
```
